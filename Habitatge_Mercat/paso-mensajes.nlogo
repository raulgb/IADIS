...
;  ]
  ...
  ask turtles [
    if tipo = "L" [
      set xcor-temp xcor
      set ycor-temp ycor
      set turtle-temp self
      set cases-aprop false
      ;; Comprovem si passen aprop d'una casa
     (foreach cases
        [ [una-casa] -> ask una-casa [
          set some_operation (((xcor - xcor-temp) * (xcor - xcor-temp) + (ycor - ycor-temp) * (ycor - ycor-temp) ) ^ (1 / 2))
          if some_operation < 3 and buida and cases-aprop = false [
            send-message (turtle-temp) "oferta" ticks
            set cases-aprop true
            set buida false
            print (word self "esta a aprop de " turtle-temp) ] ] ])
      if cases-aprop [set moviment false]
    ]
...
to process-message [sender kind message]
  ;; PER LLOGATERS
  if kind = "oferta" and tipo = "C" [
    process-pregunta-buida-message sender message
  ]

  if kind = "preu" and tipo = "C" [
   comprova-preu sender message
  ]

  ;; PER CASES
  if kind = "buida" and tipo = "L"
    [ofereix-preu sender]

  if kind = "llogar" and tipo = "L" [
    ;;demoment no fa res
  ]
; if kind = "Pong" [
